...
const NAV: NavItem[] = [
  ...
  {
    name: "Bookings",
    link: "/account-savelists", // have to change to "/owner_bookings"
    icon: EventIcon,
  },
  {
    name: "Profile",
      ...
      <div className="w-full max-w-lg flex justify-around mx-auto text-sm text-center ">
        ...
        {hasToken
          ? NAV.map((item, index) => {
              ...
              ) : item.name === "logout" ? ( // Check if the name is "logout"
                <div
                  key={index}
                  onClick={handleLogoutClick} // Call handleLogoutClick when clicked
                  className={`flex flex-col items-center justify-between text-neutral-500 dark:text-neutral-300/90 ${
                    active ? "text-neutral-900 dark:text-neutral-100" : ""
                  }`}
                >
